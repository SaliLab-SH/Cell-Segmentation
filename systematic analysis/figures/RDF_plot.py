...
def rdfplot_main( k1_, seq, output_, save_ = True, check_ = True ,fused_ = False) :
    ...
    # Save figures, (PNG,JPG,EPS,SVG,PGF and PDF supported, PDF is recommanded or PGF)
            fig.savefig("{}/{}_normalized_RDF_fused_v2.png".format(output_, seq),dpi=1200)
        else :
            fig.savefig("{}/{}_normalized_RDF_v2.png".format(output_,seq),dpi=1200)
    print('done')
...
def rdf_mask_main(datasetinfo_):
    ...
    isgindex = list(set(list(isg_tiff.reshape(1,-1))[0]))
    isgindex.remove(0)
    ...
    rdf_info_['isg num'] = len(isgindex)
