...
def read_image(datasetnum, mainpath, check_ = True):
    ...
    # return datasetnum
    return  tiff_nucleus, tiff_wholecell, tiff_mito
...
def rdfplot_main( k1_, seq, output_, save_ = True, check_ = True ,fused_ = False) :
    ...
    right = left + width
    top = bottom + height
    ax1.text(0.04*(left+right), 0.96*(bottom+top), 'NE-MITO',horizontalalignment='left',verticalalignment='center',fontsize=20,fontname="Arial",fontweight="normal",color='k',transform=ax1.transAxes)
    ...
    # Save figures, (PNG,JPG,EPS,SVG,PGF and PDF supported, PDF is recommanded or PGF)
            fig.savefig("{}/{}_normalized_mito_RDF_fused_v2.png".format(output_, seq),dpi=1200)
        else :
            fig.savefig("{}/{}_normalized_mito_RDF_v2.png".format(output_,seq),dpi=1200)
    print('done')
...
def mito_rdf(self):
    ...
    mito_position_tiff = self.mito_tiff * self.ratiomask
    mito_position_tiff2 = list(mito_position_tiff.reshape(1,-1))[0]
    print('len mito',len(mito_position_tiff2))
    mito_ratiolst = [ratio for ratio in mito_position_tiff2 if ratio != 0]
    print('len mito ratio',len(mito_ratiolst))

    
    position_num = len(mito_ratiolst)
    self._mito_volume = position_num
    
    
    mito_layernum, edges1 = np.histogram(mito_ratiolst, bins= bins_, range=(0,1))
    
    self._history['mito_layernum'] = mito_layernum
    
    average_num = len(mito_ratiolst) / self._cyto_volume
    
    
    g_average = np.zeros(len(mito_layernum))
    
    print(len(g_average))
    print(mito_layernum)
    print(self._cytolayernum)
    
    for i in range(len(g_average)):
        g_average[i] = (mito_layernum[i] / self._cytolayernum[i]) / average_num

    self._mito_g_average = g_average
    self._history['mito_g_average']  = self._mito_g_average 

    self.mito_map = True

    print(self._history['mito_g_average'])
...
def rdf_mask_main(datasetinfo_):
    ...
    temp_nc_tiff, temp_pm_tiff, tt_mito_tiff = read_image(datasetnum, mainpath, check_ = True)   
    ...
    mito_tiff = tt_mito_tiff
    mito_position_tiff = mito_tiff * ratiomask
    mrcfile = read_mrcimage(datasetnum, mrcpath, check_ = False)


    print('mito tiff size', mito_tiff.shape)
    ...
    mito_ratiolst = [ratio for ratio in mito_position_tiff2 if ratio != 0]
    print('len mito ratio',len(mito_ratiolst))
    mito_layernum, edges1 = np.histogram(mito_ratiolst, bins= 8, range=(0,1))


    position_num = len(mito_ratiolst)
    mito_volume = position_num


    average_num = len(mito_ratiolst) / cyto_volume
    
    g_average = np.zeros(len(mito_layernum))
    radii =  np.zeros(len(mito_layernum))
    ...
    for i in range(len(g_average)):
        g_average[i] = (mito_layernum[i] / cytolayernum[i]) / average_num
        radii[i] = (edges1[i] + edges1[i+1]) / 2
    
    mito_g_average = g_average


    print(mito_g_average)


    RDF_data = [radii ,mito_g_average]
    ...
    rdf_info_['mito volume'] = mito_volume
    with open(f'{outputpath}/{datasetnum}_mito_histgram_info.json', 'w') as f:
...
def multi_main_test():
    ...
    outputpath = f'F:/PBC_data/RadialDistributionFunction_X-ray_tomograms/output_mito'
