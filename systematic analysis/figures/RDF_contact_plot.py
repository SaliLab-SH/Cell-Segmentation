...
def read_image(datasetnum, mainpath, check_ = True):
    ...
    # return datasetnum
    return  tiff_nucleus, tiff_wholecell, tiff_mito, tiff_isg
...
def read_mito_image(datasetnum, mainpath, check_ = True):

    for maindir, subdir, file_name_list in os.walk(mainpath, topdown=False):
        filelist = np.array(file_name_list)

    for name in filelist:
        if f'{datasetnum}.' in name and 'isg' not in name :
            tif_name = f'{mainpath}/{name}'
            print('current tiff ',tif_name)
...
def rdfplot_main( k1_, seq, output_, save_ = True, check_ = True ,fused_ = False) :
    ...
    right = left + width
    top = bottom + height
    ax1.text(0.04*(left+right), 0.96*(bottom+top), 'NE-contact',horizontalalignment='left',verticalalignment='center',fontsize=20,fontname="Arial",fontweight="normal",color='k',transform=ax1.transAxes)
    ...
    # Save figures, (PNG,JPG,EPS,SVG,PGF and PDF supported, PDF is recommanded or PGF)
            fig.savefig("{}/{}_normalized_contact_RDF_fused_v2.png".format(output_, seq),dpi=1200)
        else :
            fig.savefig("{}/{}_normalized_contact_RDF_v2.png".format(output_,seq),dpi=1200)
    print('done')
...
def rdf_mask_main(datasetinfo_):
    ...
    mitopath = datasetinfo_['mitopath']
    ...
    temp_nc_tiff, temp_pm_tiff, tt_mito_tiff, tt_isg_tiff= read_image(datasetnum, mainpath, check_ = True)   
    temp_mito_tiff = read_mito_image(datasetnum, mitopath, check_ = True)
    temp_isg_tiff = read_isg_image(datasetnum, isgpath,  check_ = True)
    isg_ins_tiff = temp_isg_tiff * tt_isg_tiff
    ...
    mito_bkg_tiff = np.ones_like(tt_mito_tiff)
    mito_bkg_tiff[np.where(tt_mito_tiff != 0)] = 0
    mito_bkg_edt = scipy.ndimage.distance_transform_edt(mito_bkg_tiff)     

    isg_position_tiff = tt_isg_tiff * mito_bkg_edt 

    contact_position_tiff = np.zeros_like(isg_position_tiff)
    contact_position_tiff[ np.where(isg_position_tiff <= 1.5)] = 1
    contact_position_tiff[ np.where(isg_position_tiff == 0 )] = 0


    contact_position_label_tiff = skimage.morphology.label(contact_position_tiff)
    
    print('contact number', np.max(contact_position_label_tiff))
    plt.imshow(contact_position_label_tiff[int(contact_position_label_tiff.shape[0]/2)])
    plt.show()
    plt.close()


    test_index_tiff = isg_ins_tiff * contact_position_tiff
    ...
    coordlst = []
    ratiolst = []
    attached_isg_index_lst = []

    
    for index in range(1, np.max(contact_position_label_tiff)):
        allcoord = np.where(contact_position_label_tiff == index)
        coord = [int(mean(allcoord[0])), int(mean(allcoord[1])), int(mean(allcoord[2]))]
        temp = contact_position_label_tiff[allcoord]
        # print(temp)
        temp_set = list(set(temp))


        temp_coordindex = test_index_tiff[allcoord[0][0]][allcoord[1][0]][allcoord[2][0]]
        attached_isg_index_lst.append(int(temp_coordindex))


        coordlst.append(coord)
        index_lst.append(temp_set[0])
        contact_num_lst.append(len(temp))
        cores_num_lst.append(len(temp_set))
        ratiolst.append(ratiomask[coord[0]][coord[1]][coord[2]])
    
    
    # print(len(attached_isg_index_lst))
    attached_isg_index_lst = list(set(attached_isg_index_lst))
    # print(len(attached_isg_index_lst))
    ...
    position_num = len(coordlst)
    isg_contact_ratiolst = ratiolst
    

    isg_contact_layernum, edges1 = np.histogram(isg_contact_ratiolst, bins= 8  , range=(0,1))


    g_average = np.zeros(len(isg_contact_layernum))
    radii =  np.zeros(len(isg_contact_layernum))


    average_num = len(isg_contact_ratiolst) / cyto_volume

    for i in range(len(g_average)):
        g_average[i] = (isg_contact_layernum[i] / cytolayernum[i] / average_num)
        radii[i] = (edges1[i] + edges1[i+1]) / 2




    RDF_data = [list(radii) ,list(g_average)]

    # RDF_data = mask_rdf(datasetnum, temp_pm_tiff, temp_nc_tiff, isg_tiff, ratiomask, bins_ = 8)

    print(RDF_data)
    print(RDF_data[0])
    print(np.max(contact_position_label_tiff))

    rdf_info_ = {}
    rdf_info_['dataset'] = datasetnum
    rdf_info_['histgram'] = RDF_data  ## [RDF_data[0].tolist(), RDF_data[1].tolist()]
    rdf_info_['contact num'] = int(np.max(contact_position_label_tiff))
    with open(f'{outputpath}/{datasetnum}_contact_histgram_info.json', 'w') as f:
...
def multi_main_test():
    ...
    mitopath = f'F:/modelling/Cluster-bkp/merged_prediction_mask_match_mito'
    ...
    outputpath = f'F:/PBC_data/RadialDistributionFunction_X-ray_tomograms/output_contact'
